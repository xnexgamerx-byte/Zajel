...
@section('content')
<h1 class="mb-3 px-1 text-sm font-bold text-slate-600">
    ...
    <div class="mb-3 rounded-xl bg-white p-4 shadow-sm">
        <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
                ...
                <div class="font-mono text-xs text-slate-400" dir="ltr">{{ $pickup->number }}</div>
            </div>
            <div class="shrink-0 text-end">
                <div class="text-2xl font-bold">{{ $pickup->expected_count }}</div>
                <div class="text-xs text-slate-400">طرد متوقَّع</div>
            </div>
        </div>
        ...
        <dl class="mt-3 space-y-1.5 border-t border-slate-100 pt-3 text-sm">
            ...
            <div>
                ...
                <dd class="font-bold text-brand-700">
                    {{ $pickup->landmark ?: $pickup->merchant->landmark ?: '—' }}
                </dd>
            </div>
            @if ($pickup->notes)
                <div><dt class="text-xs text-slate-500">ملاحظة</dt>
                     <dd class="font-medium">{{ $pickup->notes }}</dd></div>
            @endif
        </dl>

        <a href="tel:{{ $pickup->contact_phone ?: $pickup->merchant->phone }}"
           class="mt-3 block rounded-xl bg-emerald-600 px-4 py-3 text-center text-base font-bold text-white active:bg-emerald-700">
            اتصل بالتاجر
        </a>
...
@empty
    <div class="rounded-xl bg-white p-10 text-center shadow-sm">
        <p class="font-semibold text-slate-700">ما عندك طلبات استلام.</p>
    </div>
@endforelse
@endsection
