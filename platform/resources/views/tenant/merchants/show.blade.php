...
<div class="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
    <div class="card p-4">
        ...
        <div class="mt-1 text-2xl font-bold {{ $merchant->balance >= 0 ? 'text-[var(--brand)]' : 'text-bad-700' }}">
                <span class="num">{{ number_format($merchant->balance) }}</span>
    ...
    @foreach ([
        ['مسلَّمة', 'delivered', 'text-ok-700'],
        ['قيد التنفيذ', null, 'text-info-700'],
        ['راجعة', 'returned', 'text-ink-600'],
    ] as [$label, $key, $tone])
        @php
...
<div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
    <div class="space-y-5 lg:col-span-2">
                        ...
                        <tbody class="divide-y divide-ink-100">
                            @foreach ($transactions as $tx)
                                <tr>
                                    ...
                                    <td class="py-2 font-semibold text-ok-700" dir="ltr">
                                        {{ $tx->direction === 'credit' ? number_format($tx->amount) : '' }}
                                    </td>
                                    <td class="py-2 font-semibold text-bad-700" dir="ltr">
        ...
        <section class="card p-5">
            ...
            @if ($recent->isEmpty())
                <p class="py-8 text-center text-sm text-ink-500">لا شحنات بعد.</p>
            @else
    ...
    <div class="space-y-5">
        <section class="card p-5">
            ...
            <dl class="space-y-2.5 text-sm">
                ...
                <div class="flex justify-between gap-3">
                    <dt class="text-ink-500">المنطقة</dt>
                ...
                <div class="flex justify-between gap-3">
                    <dt class="text-ink-500">طريقة الدفع</dt>
                    <dd class="text-end font-medium">
                ...
                @if ($merchant->payout_account)
                    <div class="flex justify-between gap-3">
                        <dt class="text-ink-500">الحساب</dt>
        ...
        @if ($merchant->notes)
            <section class="card p-5">
                ...
                <p class="text-sm text-ink-600">{{ $merchant->notes }}</p>
            </section>
        @endif
    </div>
</div>
