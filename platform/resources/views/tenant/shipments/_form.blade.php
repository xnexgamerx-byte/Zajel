    ...
    <div class="space-y-5 lg:col-span-2">
        ...
        <section class="card p-5">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                @if ($editing)
                    {{-- الشحنة تبقى لتاجرها: نقلها لتاجرٍ آخر شحنةٌ أخرى لا تصحيح --}}
                    <div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <div>
                    ...
                    <input id="pieces_count" name="pieces_count" type="number" min="1" max="255"
                           value="{{ old('pieces_count', $shipment?->pieces_count ?? 1) }}" class="field-input" dir="ltr" required>
                ...
                <div class="sm:col-span-3">
                    ...
                    <textarea id="notes" name="notes" rows="2" class="field-input"
                              placeholder="مثال: اتصل قبل الوصول بنصف ساعة">{{ old('notes', $shipment?->notes) }}</textarea>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select id="fees_paid_by" name="fees_paid_by" class="field-input">
                        <option value="merchant" @selected(old('fees_paid_by', $shipment?->fees_paid_by ?? 'merchant') === 'merchant')>
                            التاجر (تُخصم من مستحقّه)
                        </option>
                        <option value="customer" @selected(old('fees_paid_by', $shipment?->fees_paid_by) === 'customer')>
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-ink-500">
                        @if ($editing)
                            الحالية <span class="num">{{ number_format($shipment->delivery_fee) }}</span> د.ع — اتركه فارغاً لتبقى كما هي،
                            وتُحسب من جديد إن تغيّرت المحافظة أو المنطقة أو الوزن.
                        @else
                            اتركه فارغاً ليُحسب من تسعيرة التاجر.
                        @endif
                    </p>
                ...
                <button type="submit" class="btn-primary w-full">{{ $editing ? 'حفظ التعديل' : 'حفظ الشحنة' }}</button>
            </div>
        </section>
    </div>
</form>
